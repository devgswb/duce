...
//	public List<ProjectBoardModel> majorfind();
	...
	@Insert("INSERT INTO projectboard(Title, Content, Part, Guide, BranchNo, MajorNo, id, pDate, Hit, Video, Photo, addFile, reference) "
			+ "VALUES(#{title}, #{content}, #{part}, #{guide}, #{branchNo}, #{majorNo}, #{id}, #{pDate}, #{hit}, #{video}, #{photo}, #{addFile}, #{reference})")
	public void insert(ProjectBoardModel projectboard);
	
	@Update("UPDATE projectboard SET Title=#{title}, Content=#{content}, Part=#{part}, Guide=#{guide}, BranchNo=#{branchNo}, MajorNo=#{majorNo}, Video=#{video}, Photo=#{photo}, AddFile=#{addFile}, reference=#{reference} where pNo=#{pNo}")
	...
	// Y
	@Select("SELECT * FROM projectboard WHERE pDate Like CONCAT('%', #{mYear}, '%') ORDER BY pNo desc")
	...
	// Y & B
	@Select("SELECT * FROM projectboard WHERE pDate Like CONCAT('%', #{mYear}, '%') and BranchNo=#{branch} ORDER BY pNo desc")
	public List<ProjectBoardModel> findbyfilterYB(@Param("mYear") String mYear, @Param("branch") String branch);
	// Y & M
	@Select("SELECT * FROM projectboard WHERE pDate Like CONCAT('%', #{mYear}, '%') and MajorNo=#{major} ORDER BY pNo desc")
	public List<ProjectBoardModel> findbyfilterYM(@Param("mYear") String mYear, @Param("major") String major);
	// M & B & Y
	@Select("SELECT * FROM projectboard WHERE MajorNo=#{major} and BranchNo=#{branch} and pDate Like CONCAT('%', #{mYear}, '%') ORDER BY pNo desc")
